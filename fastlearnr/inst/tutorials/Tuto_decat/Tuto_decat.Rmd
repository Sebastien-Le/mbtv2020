...
perfumes_qda_experts<-read.csv("../../datasets/perfumes_qda_experts.csv",sep=",",header=TRUE)
...
Dans ce tutoriel les données utilisées :`parfumes_qda_experts`,  se réfèrent à des variétés de parfums. Elles concernent 12 variétés de parfums qui ont été évaluées par 12 panélistes selon 12 descripteurs sensoriels. Chaque panéliste évalue 2 fois chaque produit.

Nous pouvons avoir un apeçu des premières lignes grâce à la fonction  **head** :

```{r head, message=TRUE, warning=TRUE, exercise=TRUE}

head(parfumes_qda_experts) #aperçu des 6 premières lignes
...
res.decat <- decat(donnee, formul = ~ Product +Panelist, firstvar = 5, lastvar = length(colnames(parfumes_qda_experts)), 
    proba = 0.05, graph = FALSE)

```
```{r exdecat-solution}
res.decat <- decat(parfumes_qda_experts, formul = ~ Product +Panelist, firstvar = 5, lastvar = length(colnames(parfumes_qda_experts)), 
    proba = 0.05, graph = FALSE)
```
...
```{r names,  message = TRUE, exercise=TRUE, exercise.setup="var"}
names(res.decat)
```
